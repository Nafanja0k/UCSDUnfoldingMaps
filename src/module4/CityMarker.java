...
public class CityMarker extends SimplePointMarker {
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		int x1 = Integer.valueOf((int)Math.round(x - radius * Math.sin(Math.toRadians(30))));
		int y1 = Integer.valueOf((int)Math.round(y + radius * Math.cos(Math.toRadians(30))));
		int x2 = Integer.valueOf((int)Math.round(x));
		int y2 = Integer.valueOf((int)Math.round(y - radius));
		int x3 = Integer.valueOf((int)Math.round(x + radius * Math.sin(Math.toRadians(30))));
		int y3 = Integer.valueOf((int)Math.round(y + radius * Math.cos(Math.toRadians(30))));
